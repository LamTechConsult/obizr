...
OBizR.controller('addBizCtrl', function ($scope, $http, $filter, locationService, $state, CameraService, $ionicHistory, $ionicLoading, $rootScope, $localStorage, ProfileService, businessesService, taxonomyService) {
  ...
  $scope.initializeBizData = function () {
    ...
    $rootScope.newBizData.field_ltc_biz_category ={'und': [{tid:undefined}]}
    $rootScope.newBizData.field_ltc_biz_email = {'und': [{}]}
    $rootScope.newBizData.field_ltc_biz_description = {'und': [{}]}
    $rootScope.newBizData.field_ltc_business_keywords = {'und': [{tid:undefined}]}
  ...
  $scope.setChiefdom = function (item) {
    if (item == null)
      return
    $rootScope.Chiefdom = item
  }
  ...
  $scope.setModel = function (item) {
    console.log('setting autopcomplete model...');
    $scope.selectedItem = item
    if ($rootScope.currentFieldName == 'Category') {
      $rootScope.addBizSelectedItemCat = $scope.selectedItem.name
      $rootScope.newBizData.field_ltc_biz_category.und[0].tid = $scope.selectedItem.node.categoryid
    }else if ($rootScope.currentFieldName == 'Keywords') {
      $rootScope.addBizSelectedItemKeyw = $scope.selectedItem.keyword.keyword
      $rootScope.newBizData.field_ltc_business_keywords.und[0].tid = $scope.selectedItem.keyword.id
    }
  ...
  $scope.addBiz = function () {
    ...
    if ($rootScope.newBizData.field_ltc_biz_category.und[0].tid == undefined) {
      console.log($rootScope.newBizData.field_ltc_biz_category.und[0])
      $scope.serverErrors.push('Business category is required')
      return
    }
    if ($rootScope.newBizData.field_ltc_business_keywords.und[0].tid == undefined) {
      console.log($rootScope.newBizData.field_ltc_business_keywords.und[0])
      $scope.serverErrors.push('Business keywords are required')
      return
    }
    ...
    if ($rootScope.Chiefdom == undefined) {
      $scope.serverErrors.push('Business chiefdom is required')
      return
    }
    if ($rootScope.newBizData.field_ltc_biz_email.und[0].email == undefined) {
    ...
    $rootScope.newBizData.uid = $rootScope.currentUser.uid;
